...
public class HtmlTagHandler implements WrapperTagHandler {
    ...
    public static final String UNORDERED_LIST = "1HTML_TEXTVIEW_ESCAPED_UL_TAG";
    public static final String ORDERED_LIST = "1HTML_TEXTVIEW_ESCAPED_OL_TAG";
    public static final String LIST_ITEM = "1HTML_TEXTVIEW_ESCAPED_LI_TAG";
    public static final String A_ITEM = "1HTML_TEXTVIEW_ESCAPED_A_TAG";
    public static final String PLACEHOLDER_ITEM = "1HTML_TEXTVIEW_ESCAPED_PLACEHOLDER";
    ...
    String overrideTags(@Nullable String html) {
        ...
        html = html.replaceAll("<font", "<optFont");
        html = html.replaceAll("</font>", "</optFont>");
        html = html.replaceAll("<span", "<optSpan");
        html = html.replaceAll("</span>", "</optSpan>");
//
        html = html.replaceAll("<p", "<optP");
        html = html.replaceAll("</p>", "</optP>");
        html = html.replaceAll("<div", "<optSpan");
        html = html.replaceAll("</div>", "</optSpan>");
        html = html.replaceAll("<strong", "<optSpan");
        html = html.replaceAll("</strong>", "</optSpan>");

        html = html.replaceAll("<b", "<optSpan");
        html = html.replaceAll("</b>", "</optSpan>");
        html = html.replaceAll("<em", "<optSpan");
        html = html.replaceAll("</em>", "</optSpan>");
        html = html.replaceAll("<cite", "<optSpan");
        html = html.replaceAll("</cite>", "</optSpan>");
        html = html.replaceAll("<dfn", "<optSpan");
        html = html.replaceAll("</dfn>", "</optSpan>");
        html = html.replaceAll("<i>", "<optSpan");
        html = html.replaceAll("</i>", "</optSpan>");
        html = html.replaceAll("<big", "<optSpan");
        html = html.replaceAll("</big>", "</optSpan>");
        html = html.replaceAll("<small", "<optSpan");
        html = html.replaceAll("</small>", "</optSpan>");
        html = html.replaceAll("<blockquote", "<optSpan");
        html = html.replaceAll("</blockquote>", "</optSpan>");
        html = html.replaceAll("<tt", "<optSpan");
        html = html.replaceAll("</tt>", "</optSpan>");
        html = html.replaceAll("<a", "<optSpan");
        html = html.replaceAll("</a>", "</optSpan>");
////
        html = html.replaceAll("<u", "<optSpan");
        html = html.replaceAll("</u>", "</optSpan>");
        html = html.replaceAll("<del", "<optSpan");
        html = html.replaceAll("</del>", "</optSpan>");
        html = html.replaceAll("<s", "<optSpan");
        html = html.replaceAll("</s>", "</optSpan>");
        html = html.replaceAll("<strike", "<optSpan");
        html = html.replaceAll("</strike>", "</optSpan>");


        html = html.replaceAll("<sup", "<optSpan");
        html = html.replaceAll("</sup>", "</optSpan>");
        html = html.replaceAll("<sub", "<optSpan");
        html = html.replaceAll("</sub>", "</optSpan>");
        html = html.replaceAll("<h", "<optSpan");
        html = html.replaceAll("</h>", "</optSpan>");
        return html;
    }
    ...
    private static class Op {
        public Op(int mNumNewlines) {
            this.mNumNewlines = mNumNewlines;
        }
    ...
    public boolean handleTag(boolean opening, String tag, Editable output, Attributes attributes) {
        if (opening) {
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                ...
                start(output, new Td());
            } else if (tag.equalsIgnoreCase("optP")) {
                appendNewlines(output, 1);
                start(output, new Op(1));
            } else {
                return false;
            }
        } else {
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                ...
                Op n = (Op) getLast(output, Op.class);
                if (n != null) {
                    appendNewlines(output, n.mNumNewlines);
                    output.removeSpan(n);
                }

                end(output, Op.class, false);
            } else {
                return false;
            }
        }
