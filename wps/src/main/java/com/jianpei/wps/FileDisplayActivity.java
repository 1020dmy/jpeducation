...
public class FileDisplayActivity extends AppCompatActivity implements TbsReaderView.ReaderCallback,
    ...
    private String mFileUrl = "", fileName="";//文件url 由文件url截取的文件名 上个页面传过来用于显示的文件名
...
//        bundle.putString("filePath", "/storage/emulated/0/Download/aaa.pdf");
...
//    }
    ...
    private void openFile() {
        ...
        bundle.putString("tempPath", Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getPath());
        boolean result = mTbsReaderView.preOpen(parseFormat(fileName), false);
        if (result) {
            mTbsReaderView.openFile(bundle);
        }
